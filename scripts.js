...
scoreBasket(); //score should = 2
scoreBasket(); // score should = 4
addToPage("The score after two baskets and one three point throw: " + scoreThree()); //score should = 7 
...
function calcChange(amount){
    //calculate the number of quarters, then subtract from total change
    let quarters = Math.floor(amount / 25);
    amount = amount - (quarters * 25);

    //calculate the number of dimes, then subtract from total change
    let dimes = Math.floor(amount / 10);
    amount = amount - (dimes * 10);

    //calculate the number of nickles, then subtract from total change
...
function drawBox(numRows, numCols, boxChar){
    ...
    // check to see if boxChar was passed in

    //nested loops to draw the box
